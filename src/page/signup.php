...
function multiStepForm() {
    return {
        ...
        waNumber: '',
        waCode: '+62',
        formData: { email:'', password:'', nama:'', tanggal_lahir:'', wa:'', nik:'' },

        get lengthValid() { return this.password.length >=8 && this.password.length<=16 },
        get caseValid() { return /[a-z]/.test(this.password) && /[A-Z]/.test(this.password) },
        get numberValid() { return /\d/.test(this.password) },
        get matchValid() { return this.password && this.password===this.confirm },
        get isValid() { return this.lengthValid && this.caseValid && this.numberValid && this.matchValid },

        updateWA() { this.formData.wa = this.waCode + this.waNumber },

        goNext() {
            if(this.isValid) { this.formData.password = this.password; this.step=2 }
            else Swal.fire({ title:"Error", text:"Password tidak valid!", icon:"error", confirmButtonText:"Ok" })
        },
        ...
        handleSubmit() {
            ...
            if(!this.formData.email || !this.formData.password || !this.formData.nama || !this.formData.tanggal_lahir || !this.formData.wa || !this.formData.nik){
                Swal.fire({ title:"Error", text:"Lengkapi semua data!", icon:"error", confirmButtonText:"Ok" });
                return;
            }
